...
<body>
  <form class="m-l-15 m-r-15" runat="server">
    ...
    <div class="card-box">
      <div class="m-t-0 header-title"> 修改文件访问地址 </div>
      <p class="text-muted font-13 m-b-25"> 站点名称： <asp:Literal id="LtlSiteName" runat="server"></asp:Literal>
      </p>
      <div class="form-group">
        <label class="col-form-label">文件部署方式</label>
        <asp:RadioButtonList id="RblIsSeparatedAssets" RepeatDirection="Horizontal" AutoPostBack="true"
          OnSelectedIndexChanged="RblIsSeparatedAssets_SelectedIndexChanged" class="radio radio-primary" runat="server"></asp:RadioButtonList>
        <small class="form-text text-muted">文件包含图片、视频、附件等除了HTML页面之外的所有文件</small>
      </div>
      <asp:PlaceHolder ID="PhSeparatedAssets" runat="server">
        <div class="form-group">
          <label class="col-form-label">文件存储文件夹
            <asp:RegularExpressionValidator runat="server" ControlToValidate="TbAssetsDir" ValidationExpression="[^']+"
              ErrorMessage=" *" ForeColor="red" Display="Dynamic" />
          </label>
          <asp:TextBox id="TbAssetsDir" class="form-control" runat="server"></asp:TextBox>
        </div>
        <div class="form-group">
          <label class="col-form-label">独立部署文件访问地址
            <asp:RequiredFieldValidator ControlToValidate="TbSeparatedAssetsUrl" ErrorMessage=" *" ForeColor="red"
              Display="Dynamic" runat="server" />
            <asp:RegularExpressionValidator runat="server" ControlToValidate="TbSeparatedAssetsUrl"
